...
import org.apache.hadoop.io.{DataOutputBuffer, Writable}
 ...
 *
 */
abstract class MediaWikiRevisionXMLTransformerInputFormatAbstract
  [K<: Writable, V<: Writable, TOF <: MediaWikiObjectsFactory](val objectsFactory: TOF)
  extends FileInputFormat[K, V] with KeyValueFactory[K, V, TOF] {

  private val LOG = LogFactory.getLog(classOf[MediaWikiRevisionXMLTransformerInputFormatAbstract[K, V, TOF]])
  ...
  override def createRecordReader(genericSplit: InputSplit, context: TaskAttemptContext): RecordReader[K, V] =
  {
    val split = genericSplit.asInstanceOf[FileSplit]
    LOG.info("getRecordReader start.....split=" + split)
    context.setStatus(split.toString)

    new WikiRevisionXMLTransformerRecordReaderAbstract(split, context, objectsFactory)
  }

  private class WikiRevisionXMLTransformerRecordReaderAbstract
    (split: FileSplit, context: TaskAttemptContext, objectsFactory: TOF)
    ...
    override def nextKeyValue(): Boolean = {
      ...
      if (matcher.readUntilMatch(revisionEndPattern, Some(contentBuffer))) {
        ...
        val rev = mediaWikiXMLParser.parseRevision(mediaWikiXMLParser.initializeXmlStreamReader(byteArrayInputStream), pageMetaData)
        setKey(key, rev)
        setValue(value, rev)
        contentBuffer.reset()
        return !filterKeyValue(key, value)
      }
      contentBuffer.reset()
      false
    }
